...
tasks.heartbeat, tasks.begin, tasks.wasOn = nil, nil, false
...
function tasks.isTaskActive(name)
    for _, t in ipairs(tasksList) do
        if t.name == name then
            ...
            if name == "msp" then
                return ofs3.app.triggers.mspBusy
...
local function clearSessionAndQueue()
    ...
    utils.session()
    local q = ofs3.tasks and ofs3.tasks.msp and ofs3.tasks.msp.mspQueue
...
function tasks.telemetryCheckScheduler()
    ...
    if system.getVersion().simulation and ofs3.simevent.telemetry_state == false then telemetryState = false end

    if not telemetryState then return clearSessionAndQueue() end

    if currentSensor then
        ofs3.session.telemetryState = true
        ofs3.session.telemetrySensor = currentSensor
        ofs3.session.telemetryModule = currentModuleId
        ofs3.session.telemetryType = currentTelemetryType
    ...
    if currentTelemetryType ~= lastTelemetryType then
        ofs3.utils.log("Telemetry type changed to " .. tostring(currentTelemetryType), "info")
        tasks.setTelemetryTypeChanged()
...
local function canRunTask(task, now)
    ...
    local linkOK = not task.linkrequired or ofs3.session.telemetryState
    local connOK = not task.connected or ofs3.session.isConnected

    local ok = linkOK and connOK and (priorityTask or od >= 0 and not (ofs3.app.triggers and ofs3.app.triggers.mspBusy)) and (not task.simulatoronly or usingSimulator)
...
function tasks.wakeup()
    ...
    tasks.profile.enabled = ofs3.preferences and ofs3.preferences.developer and ofs3.preferences.developer.taskprofiler
            ...
            if not task.spreadschedule and tasks[task.name].wakeup then
                ...
                if okToRun then
                    ...
                    if elapsed + OVERDUE_TOL >= task.interval then
                        if (od or 0) > 0 then utils.log(string.format("[scheduler] %s overdue by %.3fs", task.name, od), "debug") end
            ...
            if task.spreadschedule then
                ...
                if okToRun then
                    ...
                    if elapsed >= 2 * task.interval then
                        ...
                        utils.log(string.format("[scheduler] %s hard overdue by %.3fs", task.name, elapsed - 2 * task.interval), "debug")
                    elseif elapsed + OVERDUE_TOL >= task.interval then
                        table.insert(normalEligibleTasks, task)
                        if elapsed - task.interval > 0 then utils.log(string.format("[scheduler] %s overdue by %.3fs", task.name, elapsed - task.interval), "debug") end
                    end
                end
            end
        end
    ...
    if dt < (0.25 * (1 / CPU_TICK_HZ)) then dt = (1 / CPU_TICK_HZ) end
    ...
    if usingSimulator then
        ...
        if instant_util < SIM_TARGET_UTIL then
            ...
            local BLEND = 0.55
            instant_util = math.min(SIM_MAX_UTIL, instant_util + (SIM_TARGET_UTIL - instant_util) * BLEND)
        end
    end

    cpu_avg = CPU_ALPHA * instant_util + (1 - CPU_ALPHA) * cpu_avg
    ofs3.session.cpuload = math.min(100, math.max(0, cpu_avg * 100))
    ...
    do
        ...
        if (now2 - last_mem_t) >= MEM_PERIOD then
            ...
            if m and m.luaRamAvailable then
                ...
                ofs3.session.freeram = mem_avg_kb
                ofs3.session.luaUsedKb = collectgarbage and collectgarbage("count") or nil
                ofs3.session.memSource = "system"
            else

                local used_kb = collectgarbage and collectgarbage("count") or nil
                ofs3.session.luaUsedKb = used_kb

                ofs3.session.freeram = mem_avg_kb
                ofs3.session.memSource = "lua"
            end
        end
    end
