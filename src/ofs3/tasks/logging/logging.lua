...
local logTable = {
    {name = "voltage", keyindex = 1, keyname = "Voltage", keyunit = "v", keyminmax = 1, color = colorTable['voltage'], pen = SOLID, graph = true}, {name = "current", keyindex = 2, keyname = "Current", keyunit = "A", keyminmax = 1, color = colorTable['current'], pen = SOLID, graph = true},
    ...
    {name = "throttle_percent", keyindex = 5, keyname = "Throttle %", keyunit = "%", keyminmax = 1, color = colorTable['throttle_percent'], pen = SOLID, graph = false}
}
...
function logging.writeLogs(forcewrite)
    ...
    if #log_queue > 0 and logFileName then
        local filePath = "LOGS:ofs3/telemetry/" .. logFileName

        ofs3.utils.log(string.format("Write %d (of %d) lines to %s", math.min(#log_queue, max_lines), #log_queue, logFileName), "info")
...
function logging.getLogHeader()
    ...
    return "time, " .. ofs3.utils.joinTableItems(names, ", ")
end
...
function logging.getLogLine()
    ...
    local ts = os.time()
    return ts .. ", " .. ofs3.utils.joinTableItems(values, ", ")
end
...
function logging.flushLogs()
    if logFileName or logHeader then
        ofs3.utils.log("Flushing logs - " .. tostring(logFileName), "info")
...
function logging.wakeup()
    ...
    if not telemetry then
        telemetry = ofs3.tasks.telemetry
        return
    end
    ...
    if ofs3.utils.inFlight() then
        if not logFileName then
            logFileName = generateLogFilename()
            ofs3.utils.log("Logging triggered by inFlight() - " .. logFileName, "info")

            local iniName = "LOGS:ofs3/telemetry/logs.ini"
            local iniData = ofs3.ini.load_ini_file(iniName) or {}
            if not iniData.model then iniData.model = {} end
            iniData.model.name = ofs3.session.craftName or model.name() or "Unknown"
            ofs3.ini.save_ini_file(iniName, iniData)
        end
