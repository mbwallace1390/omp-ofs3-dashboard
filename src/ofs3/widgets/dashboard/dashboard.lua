...
dashboard.flightmode = ofs3.flightmode.current or "preflight"
...
function dashboard.loadObjectType(box)
    ...
    if not dashboard._moduleCache[typ] then
        ...
        local ok, obj = pcall(function() return assert(loadfile(objPath))() end)
...
function dashboard.computeOverlayMessage()
    ...
    if not tasks.active() then return "@i18n(widgets.dashboard.check_bg_task)@" end

    if ofs3.session.apiVersion and ofs3.session.rfVersion and not ofs3.session.isConnectedLow and state ~= "postflight" then
        if system.getVersion().simulation == true then
            return pad .. "SIM " .. ofs3.session.apiVersion .. pad
        else
            return pad .. "RF" .. ofs3.session.rfVersion .. pad
        end
    end

    if not ofs3.session.isConnectedHigh and state ~= "postflight" then return "@i18n(widgets.dashboard.waiting_for_connection)@" end
...
function dashboard.renderLayout(widget, config)
    ...
    if layout.showgrid or ofs3.preferences.developer.overlaygrid then
    ...
    if layout.showstats or ofs3.preferences.developer.overlaystats then
        local headerOffset = (isFullScreen and headerLayout and headerLayout.height) or 0

        local cpuUsage = (ofs3.performance and ofs3.performance.cpuload) or 0
        local loopMs = (ofs3.performance and ofs3.performance.loop_ms) or 0
        local budgetMs = (ofs3.performance and ofs3.performance.budget_ms) or 50
        local tickMs = (ofs3.performance and ofs3.performance.tick_ms)
        local headroomPct = math.max(0, 100 - (cpuUsage or 0))

        local ramFreeKB = (ofs3.performance and ofs3.performance.luaRamKB) or 0
        local ramUsedGC_KB = (ofs3.performance and ofs3.performance.usedram) or 0
        local sysRamFreeKB = (ofs3.performance and ofs3.performance.ramKB) or 0
        local bitmapRamFreeKB = (ofs3.performance and ofs3.performance.luaBitmapsRamKB) or 0
        local mainStackKB = (ofs3.performance and ofs3.performance.mainStackKB) or 0
        ...
        local function fmtPct(n) return ofs3.utils.round(n or 0, 0) end
        ...
        local schedRows = {{"LOAD", fmtPct(cpuUsage), "%"}, {"LOAD (100ms window)", fmtPct(ofs3.performance.cpuload_window100 or 0), "%"}, {"HEADROOM", fmtPct(headroomPct), "%"}, {"LOOP / BUDGET", fmtMS(loopMs) .. " / " .. fmtMS(budgetMs), "ms"}}
...
local function getThemeForState(state)
    local prefs = ofs3.session.modelPreferences and ofs3.session.modelPreferences.dashboard
    local fallback = ofs3.preferences.dashboard
...
local function getThemeForState(state)
    local modelPrefs = ofs3.session.modelPreferences and ofs3.session.modelPreferences.dashboard
    local userPrefs = ofs3.preferences.dashboard
    local val = nil
    if modelPrefs then
...
function dashboard.create()
    ...
    if not dashboard.utils then dashboard.utils = assert(loadfile("SCRIPTS:/" .. ofs3.config.baseDir .. "/widgets/dashboard/lib/utils.lua"))() end
    if not dashboard.loaders then dashboard.loaders = assert(loadfile("SCRIPTS:/" .. ofs3.config.baseDir .. "/widgets/dashboard/lib/loaders.lua"))() end

    os.mkdir("SCRIPTS:/" .. ofs3.config.preferences .. "/dashboard/")
...
function dashboard.event(widget, category, value, x, y)
    ...
    if state == "postflight" and category == EVT_KEY and value == 131 then
        ofs3.widgets.dashboard.flightmode = "preflight"
...
function dashboard.wakeup(widget)
    ...
    if ofs3.session and ofs3.session.mspBusy and not (ofs3.session and ofs3.session.isConnected) then return end

    local now = os.clock()
    local visible = lcd.isVisible()
    local admin = ofs3.app and ofs3.app.guiIsRunning
    ...
    objectProfiler = ofs3.preferences and ofs3.preferences.developer and ofs3.preferences.developer.logobjprof
    ...
    if firstWakeupCustomTheme and ofs3.session.mcu_id and ofs3.session.modelPreferences and ofs3.session.modelPreferences.dashboard then

        local modelPrefs = ofs3.session.modelPreferences.dashboard
        local currentPrefs = ofs3.preferences.dashboard
    ...
    local currentFlightMode = ofs3.flightmode.current or "preflight"
                ...
                if utils.dir_exists(basePath, folder) then
                    ...
                    if chunk then
                        ...
                        if ok and initTable and type(initTable.name) == "string" then
                            if not initTable.developer or ofs3.preferences.developer.devtools == true then
                                num = num + 1
...
function dashboard.getPreference(key)
    if not ofs3.session.modelPreferences or not dashboard.currentWidgetPath then return nil end

    if not ofs3.app.guiIsRunning then
        return ofs3.ini.getvalue(ofs3.session.modelPreferences, dashboard.currentWidgetPath, key)
    else
        return ofs3.ini.getvalue(ofs3.session.modelPreferences, ofs3.app.dashboardEditingTheme, key)
    end
end

function dashboard.savePreference(key, value)
    if not ofs3.session.modelPreferences or not ofs3.session.modelPreferencesFile or not dashboard.currentWidgetPath then return false end
    if not ofs3.app.guiIsRunning then
        ofs3.ini.setvalue(ofs3.session.modelPreferences, dashboard.currentWidgetPath, key, value)
        return ofs3.ini.save_ini_file(ofs3.session.modelPreferencesFile, ofs3.session.modelPreferences)
    else
        ofs3.ini.setvalue(ofs3.session.modelPreferences, ofs3.app.dashboardEditingTheme, key, value)
        return ofs3.ini.save_ini_file(ofs3.session.modelPreferencesFile, ofs3.session.modelPreferences)
    end
end
