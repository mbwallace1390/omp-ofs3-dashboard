...
ofs3.session.dialImageCache = ofs3.session.dialImageCache or {}
local function loadDialPanelCached(dialId)
    local key = tostring(dialId or "panel1")
    if not ofs3.session.dialImageCache[key] then
        local panelPath = resolveDialAsset(dialId, "widgets/dashboard/gfx/dials") or "widgets/dashboard/gfx/dials/panel1.png"
        ofs3.session.dialImageCache[key] = ofs3.utils.loadImage(panelPath)
    end
    return ofs3.session.dialImageCache[key]
end
