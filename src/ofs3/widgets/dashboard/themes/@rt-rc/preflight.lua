...
local function wakeup(box, telemetry)
    ...
    if not (ofs3.session and ofs3.session.telemetryState) then
        cache.status, cache.statusColor, cache.statusSub, cache.issueText = "WAITING", C.muted, "CONNECT TELEMETRY", nil
        return cache
    end
    if cache.isArmed == true then faults = faults + 1; issues[#issues + 1] = "MODEL IS ARMED" end
    if cache.fuel ~= nil then
        available = available + 1
        if cache.fuel <= common.getConfig("fuel_warn") then warnings = warnings + 1; issues[#issues + 1] = "SMART FUEL " .. common.formatValue(cache.fuel, 0, "%") .. " AT RESERVE" end
    end
    if cache.voltage ~= nil then
        available = available + 1
        if cache.voltage < criticalPack then faults = faults + 1; issues[#issues + 1] = "PACK " .. common.formatValue(cache.voltage, 1, "V") .. " BELOW MINIMUM"
        elseif cache.voltage < warningPack then warnings = warnings + 1; issues[#issues + 1] = "PACK " .. common.formatValue(cache.voltage, 1, "V") .. " LOW" end
    end
    if cache.esc ~= nil then
        available = available + 1
        if cache.esc >= common.getConfig("esc_max") then faults = faults + 1; issues[#issues + 1] = "ESC " .. common.formatValue(cache.esc, 0, "°C") .. " AT LIMIT"
        elseif cache.esc >= common.getConfig("esc_warn") then warnings = warnings + 1; issues[#issues + 1] = "ESC " .. common.formatValue(cache.esc, 0, "°C") .. " ABOVE WARNING" end
    end
    if cache.link ~= nil then available = available + 1; if cache.link < common.getConfig("link_warn") then warnings = warnings + 1; issues[#issues + 1] = "LINK " .. common.formatValue(cache.link, 0, "%") .. " LOW" end end

    local issueCount = faults + warnings
    cache.issueText = issues[1]
    if issueCount > 1 and cache.issueText then cache.issueText = cache.issueText .. "  +" .. tostring(issueCount - 1) .. " MORE" end
    if available == 0 then cache.status, cache.statusColor, cache.statusSub, cache.issueText = "WAITING", C.muted, "WAITING FOR SENSOR DATA", nil
    elseif faults > 0 then cache.status, cache.statusColor, cache.statusSub = "CHECK", C.red, tostring(issueCount) .. " ITEM" .. (issueCount == 1 and "" or "S") .. " FLAGGED"
    elseif warnings > 0 then cache.status, cache.statusColor, cache.statusSub = "CAUTION", C.amber, tostring(issueCount) .. " ITEM" .. (issueCount == 1 and "" or "S") .. " TO REVIEW"
    else cache.status, cache.statusColor, cache.statusSub, cache.issueText = "READY", C.green, "SYSTEMS NOMINAL", nil end
    return cache
end

local function drawCheckRow(x, y, w, label, value, stateColor, compact)
    lcd.color(stateColor); lcd.drawFilledRectangle(floor(x), floor(y + 5), 7, 7)
...
local function paint(x, y, w, h, box, cache)
    ...
    local fuelColor, packColor, escColor, linkColor = common.fuelColor(cache.fuel), common.packColor(cache.voltage), common.escColor(cache.esc), common.linkColor(cache.link)
    ...
    local cy = compact and (bodyY + bodyH * 0.29) or (bodyY + bodyH * 0.42)
    local radius = compact and min(centerW * 0.25, bodyH * 0.22) or min(centerW * 0.33, bodyH * 0.32)
    ...
    else common.drawTextAligned(centerX, cy + (compact and 17 or 22), centerW, cache.statusSub or "CONNECT TELEMETRY", "FONT_XXS", cache.statusColor or C.muted, "center") end
    ...
    lcd.color(fuelColor); lcd.drawFilledRectangle(floor(centerX + centerW - 20), floor(segmentsY + (compact and 3 or 5)), 5, 8)
...
return {layout=layout,boxes=boxes,header_boxes=header_boxes,header_layout=header_layout,scheduler={spread_scheduling=true,spread_scheduling_paint=false,spread_ratio=0.85}}
