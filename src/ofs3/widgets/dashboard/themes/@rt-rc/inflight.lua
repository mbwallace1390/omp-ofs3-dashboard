...
local function drawRadialGauge(cx, cy, radius, value, maximum, color)
    local startAngle, sweep, ticks = 140, 260, 32
    ...
    for index = 0, ticks - 1 do
        ...
        local x1, y1 = cx + math.cos(angle) * inner, cy + math.sin(angle) * inner
        local x2, y2 = cx + math.cos(angle) * radius, cy + math.sin(angle) * radius
...
local function drawVerticalMeter(x, y, w, h, title, value, maximum, color, unit, compact)
    ...
    local barX, barY, barW = x + 15, y + (compact and 30 or 34), 14
    local barH = h - (compact and 44 or 52)
    local percent = value and maximum > 0 and common.clamp(value / maximum, 0, 1) or 0
    lcd.color(C.line); lcd.drawRectangle(floor(barX), floor(barY), floor(barW), floor(barH), 1)
    if percent > 0 then
        local fillHeight = floor((barH - 4) * percent)
        lcd.color(color); lcd.drawFilledRectangle(floor(barX + 2), floor(barY + barH - 2 - fillHeight), floor(barW - 4), fillHeight)
    end
...
local function drawCompactMetric(x, y, w, h, title, valueText, accent, subtitle)
    ...
    if h < 70 then common.drawTextAligned(x + 12, y + 23, w - 24, valueText, "FONT_S", C.white, "left")
    else
        common.drawTextAligned(x + 12, y + 26, w - 24, valueText, "FONT_L", C.white, "left")
        if subtitle then common.drawTextAligned(x + 12, y + h - 22, w - 24, subtitle, "FONT_XXS", C.muted, "left") end
    end
end
...
local function paint(x, y, w, h, box, cache)
    ...
    local leftW, rightW = floor(w * 0.18), floor(w * 0.24)
    local centerX = x + pad + leftW + pad
    local centerW = w - leftW - rightW - pad * 4
    local leftX, rightX = x + pad, centerX + centerW + pad
    local escColor = common.escColor(cache.esc)
    local throttleColor = (cache.throttle or 0) >= 90 and C.amber or C.cyan
    local fuelColor, packColor, linkColor = common.fuelColor(cache.fuel), common.packColor(cache.voltage), common.linkColor(cache.link)
    ...
    lcd.color(fuelColor); lcd.drawFilledRectangle(floor(rightX + rightW - 16), floor(bodyY + fuelH - (compact and 28 or 35)), 4, 8)
    local stateGap, stateH = compact and 6 or 8, compact and 25 or 28
    ...
    local consumedX, consumedW = leftX + 38, leftW - 50
    ...
    if not compact then common.drawTextAligned(x + w * 0.67, y + h - 22, w * 0.31 - pad, "AEGIS MONITORING", "FONT_XXS", C.line2, "right") end
end
