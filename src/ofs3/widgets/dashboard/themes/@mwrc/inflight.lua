--[[
  ...
  (throttle_percent, smartconsumption, governor-based armed state)
  have been substituted for real ofs3 sensors (current, consumption,
  armed) - see the PR description for the full substitution list.
]] --
    ...
    return {
        {
            ...
            wakeup = function(box, telemetry)
                ...
                c.isArmed = false
                if telemetry and telemetry.getSensor then
                    local arm = telemetry.getSensor("armed")
                    if arm ~= nil and arm == 0 then c.isArmed = true end
                end
                box._cache = c
                return c
            end,
        ...
        },
        {
            ...
            type = "func", subtype = "func", wakeup = wakeAesGauge, paint = paintAesGauge,
            bgcolor = "transparent",
            source = "current", arcmax = true, unit = "A", accentcolor = rc.green, glowcolor = rc.greenDim,
            title = "CURRENT", titlepos = "bottom", titlefont = arcTitleFont,
            min = 0, max = getThemeValue("current_max"), thickness = math.max(3, math.floor(opts.thickness * 0.3)),
            font = "FONT_XL", maxfont = arcMaxFont,
            thresholds = {{value = getThemeValue("current_max") * 0.7, fillcolor = rc.green}, {value = getThemeValue("current_max") * 0.85, fillcolor = rc.amber}, {value = 999999, fillcolor = rc.red}}
        },
        {
        ...
        },
        {
            ...
            title = "STATUS", font = "FONT_S", titlefont = "FONT_XS", bgcolor = "transparent"
        },
        {
        ...
        },
        {
            ...
            type = "text", subtype = "telemetry", source = "consumption",
            title = "MAH", titlepos = "bottom", unit = "",
            font = "FONT_S", titlefont = "FONT_XS", bgcolor = "transparent",
            titlecolor = rc.cyan, textcolor = rc.white, transform = "floor"
        }
    }
end
