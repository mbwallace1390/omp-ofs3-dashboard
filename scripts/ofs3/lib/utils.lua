...
function utils.session()
    ...
    ofs3.session.modelPreferencesFile = nil
    ...
    ofs3.session.onConnect = {}
    ofs3.session.onConnect.high = false
    ofs3.session.onConnect.low = false
    ofs3.session.onConnect.medium = false
    ofs3.session.rx = {}
    ofs3.session.rx.map = {}
    ofs3.session.rx.values = {}
end

function utils.rxmapReady()

    if ofs3.session.rx and ofs3.session.rx.map and (ofs3.session.rx.map.collective or ofs3.session.rx.map.elevator or ofs3.session.rx.map.throttle or ofs3.session.rx.map.rudder) then return true end
    return false
end

function utils.inFlight()
    if ofs3.flightmode.current == "inflight" then return true end
    return false
end
...
function utils.getGovernorState(value)
    ...
    if not ofs3.tasks.telemetry then return "@i18n(widgets.governor.UNKNOWN)@" end
...
function utils.logRotorFlightBanner()
    ...
    local banner = {
        "===============================================", "    ROTORFLIGHT OFS3 - Version: " .. version, "===============================================", "   ______.........--=T=--.........______", "      .             |:|", " :-. //           /\"\"\"\"\"\"-.",
        " ': '-._____..--\"\"(\"\"\"\"\"\")()`---.__", "  /:   _..__   ''  \":\"\"\"\"'[] |\"\"`\\\\", "  ': :'     `-.     _:._     '\"\"\"\" :", "   ::          '--=:____:.___....-\"", "                     O\"       O\"", "===============================================",
        "  ofs3 is free software licensed under", "  the GNU General Public License version 3.0", "  https://www.gnu.org/licenses/gpl-3.0.en.html", "                                              ", " For more information, visit rotorflight.org", "==============================================="
    }

    for _, line in ipairs(banner) do ofs3.utils.log(line, "info") end
end
...
function utils.log(msg, level) if ofs3.tasks and ofs3.tasks.logger then ofs3.tasks.logger.add(msg, level or "debug") end end
...
function utils.findWidgets()
    ...
    for _, v in pairs(system.listFiles(widgets_path)) do
        ...
        if v ~= ".." and v ~= "." and not v:match("%.%a+$") then
            ...
            local func, err = loadfile(init_path)
            if not func then
                ofs3.utils.log("Failed to load widget init " .. init_path .. ": " .. err, "debug")
            else
                local ok, wconfig = pcall(func)
                if not ok then
                    ofs3.utils.log("Error executing widget init " .. init_path .. ": " .. wconfig, "debug")
                elseif type(wconfig) ~= "table" or not wconfig.key then
                    ofs3.utils.log("Invalid configuration in " .. init_path, "debug")
                else
                    wconfig.folder = v
...
function utils.logMsp(cmd, rwState, buf, err)
    if ofs3.preferences.developer.logmsp then
        ...
        if err then ofs3.utils.log("Error: " .. err, "info") end
    end
end
...
function utils.reportMemoryUsage(location)
    ...
    if ofs3.preferences.developer.memstats == false then return end
