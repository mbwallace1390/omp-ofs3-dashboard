...
function utils.session()
    ofs3.session = {}
    ofs3.session.tailMode = nil
    ofs3.session.swashMode = nil
    ofs3.session.activeProfile = nil
    ofs3.session.activeRateProfile = nil
    ofs3.session.activeProfileLast = nil
    ofs3.session.activeRateLast = nil
    ofs3.session.servoCount = nil
    ofs3.session.servoOverride = nil
    ofs3.session.clockSet = nil
    ...
    ofs3.session.tailMode = nil
    ofs3.session.swashMode = nil
    ofs3.session.formLineCnt = nil
    ofs3.session.rateProfile = nil
    ofs3.session.governorMode = nil
    ofs3.session.servoOverride = nil
    ofs3.session.ethosRunningVersion = nil
    ofs3.session.lcdWidth = nil
    ofs3.session.lcdHeight = nil
    ofs3.session.mspSignature = nil
    ofs3.session.telemetryState = nil
    ofs3.session.telemetryType = nil
    ofs3.session.telemetryTypeChanged = nil
    ofs3.session.telemetrySensor = nil
    ofs3.session.repairSensors = false
    ofs3.session.locale = system.getLocale()
    ofs3.session.lastMemoryUsage = nil
    ofs3.session.mcu_id = nil
    ofs3.session.isConnected = false
    ofs3.session.isArmed = false
    ofs3.session.bblSize = nil
    ofs3.session.bblUsed = nil
    ofs3.session.batteryConfig = nil
    -- keep ofs3.session.batteryConfig nil as it is used to determine if the battery config has been loaded
    -- ofs3.session.batteryConfig  will end up containing the following:
        -- batteryCapacity = nil
    ...
    ofs3.session.modelPreferences = nil -- this is used to store the model preferences
    ofs3.session.modelPreferencesFile = nil -- this is used to store the model preferences file path
    ofs3.session.dashboardEditingTheme = nil -- this is used to store the dashboard theme being edited in settings
    ofs3.session.timer = {}
    ...
    ofs3.session.onConnect = {} -- this is used to store the onConnect tasks that need to be run
    ofs3.session.onConnect.high = false
    ofs3.session.onConnect.low = false
    ofs3.session.onConnect.medium = false
    ofs3.session.rx = {}
    ofs3.session.rx.map = {}
    ofs3.session.rx.values = {} -- this is used to store the rx values for the rxmap task
end

--- Checks if the RX map is ready by verifying the presence of required channel mappings.
-- The function returns true if the `ofs3.session.rxmap` table exists and at least one of the following fields is present:
...
function utils.rxmapReady()
    ...
    if ofs3.session.rx and ofs3.session.rx.map  and (ofs3.session.rx.map.collective or ofs3.session.rx.map.elevator or ofs3.session.rx.map.throttle or ofs3.session.rx.map.rudder) then
        return true
    end
    return false
end
...
function utils.inFlight()
    if ofs3.flightmode.current == "inflight" then
        return true
    end
    return false
end
...
function utils.msp_version_array_to_indexed()
    local arr = {}
    local tbl = ofs3.config.supportedMspApiVersion or {"12.06", "12.07","12.08"} 
...
function utils.getGovernorState(value)
    ...
    if ofs3.session and ofs3.session.apiVersion and ofs3.session.apiVersion > 12.07 then
        local armflags = ofs3.tasks.telemetry.getSensor("armflags")
    ...
    --]]
    local armdisableflags = ofs3.tasks.telemetry.getSensor("armdisableflags")
...
function utils.createCacheFile(tbl, path, options)
    ...
    if not f then
        ofs3.utils.log("Error creating cache file: " .. err, "info")
        return
    end
...
function utils.logRotorFlightBanner()
    local version = ofs3.version().version or "Unknown Version"

    local banner = {
        "===============================================",
        "    ROTORFLIGHT OFS3 - Version: " .. version,
        "===============================================",
        "   ______.........--=T=--.........______",
        "      .             |:|",
        " :-. //           /\"\"\"\"\"\"-.",
        ...
        "                     O\"       O\"",
        "===============================================",
        "  ofs3 is free software licensed under",
...
function utils.playFile(pkg, file)
    ...
    local wavUser   = "SCRIPTS:/ofs3.user/audio/user/"      .. pkg .. "/" .. file
    local wavLocale = "SCRIPTS:/ofs3.user/audio/" .. av .. "/" .. pkg .. "/" .. file
    local wavDefault= "SCRIPTS:/ofs3/audio/en/default/"    .. pkg .. "/" .. file

    -- Determine which file to play: user → locale → default
    local path
    if ofs3.utils.file_exists(wavUser) then
        path = wavUser
    elseif ofs3.utils.file_exists(wavLocale) then
        path = wavLocale
    else
        path = wavDefault
    end
...
function utils.getCurrentProfile()
    ...
    local pidProfile = ofs3.tasks.telemetry.getSensor("pid_profile")
    local rateProfile = ofs3.tasks.telemetry.getSensor("rate_profile")

    if (pidProfile ~= nil and rateProfile ~= nil) then

        ofs3.session.activeProfileLast = ofs3.session.activeProfile
        local p = pidProfile
        if p ~= nil then
            ofs3.session.activeProfile = math.floor(p)
        else
            ofs3.session.activeProfile = nil
        end

        ofs3.session.activeRateProfileLast = ofs3.session.activeRateProfile
        local r = rateProfile
        if r ~= nil then
            ofs3.session.activeRateProfile = math.floor(r)
        else
            ofs3.session.activeRateProfile = nil
        end
...
function utils.ethosVersionAtLeast(targetVersion)
    ...
    if targetVersion == nil then 
        if ofs3 and ofs3.config and ofs3.config.ethosVersion then
            targetVersion = ofs3.config.ethosVersion
        else
            -- Fail-safe: if no targetVersion is provided and config is missing
            return false
        end
    elseif type(targetVersion) == "number" then
        ofs3.utils.log("WARNING: utils.ethosVersionAtLeast() called with a number instead of a table (" .. targetVersion .. ")",2)
        return false    
    end
...
function utils.log(msg, level)
    if ofs3.tasks and ofs3.tasks.logger then
        ofs3.tasks.logger.add(msg, level or "debug")
    end
end
    ...
    for _, v in pairs(system.listFiles(modules_path)) do
        ...
        if v ~= ".." and v ~= "." and not v:match("%.%a+$") then
            ...
            if not func then
                ...
                if not ok then
                    ofs3.utils.log("Error executing " .. init_path .. ": " .. mconfig, "info")
                elseif type(mconfig) ~= "table" or not mconfig.script then
                    ofs3.utils.log("Invalid configuration in " .. init_path, "info")
                else
                    ofs3.utils.log("Loading module " .. v, "debug")
                    mconfig.folder = v
    ...
    local function getCachedBitmap(key, tryPaths)
        ...
        if not path then
            for _, p in ipairs(tryPaths) do
                if ofs3.utils.file_exists(p) then
                    path = p
                    break
                end
            end
...
function utils.logMsp(cmd, rwState, buf, err)
    if ofs3.preferences.developer.logmsp then
        local payload = ofs3.utils.joinTableItems(buf, ", ")
        ofs3.utils.log(rwState .. " [" .. cmd .. "]" .. " {" .. payload .. "}", "info")
        if err then
            ofs3.utils.log("Error: " .. err, "info")
        end
    end
end
...
function utils.reportMemoryUsage(location)
    ...
    if ofs3.preferences.developer.memstats == false then
        return
    end
    ...
    local lastMemoryUsage = ofs3.session.lastMemoryUsage
    ...
    -- Log the message
    ofs3.utils.log(logMessage, "info")

    -- Store the current memory usage in bytes for future calls (convert back to bytes)
    ofs3.session.lastMemoryUsage = system.getMemoryUsage().luaRamAvailable
end


function utils.onReboot()
    ofs3.session.resetSensors = true
    ofs3.session.resetTelemetry = true
    ofs3.session.resetMSP = true
