...
function dashboard.computeOverlayMessage()
    ...
    if not ofs3.session.isConnectedHigh and state ~= "postflight" then
...
function dashboard.renderLayout(widget, config)
    ...
    if layout.showstats or ofs3.preferences.developer.overlaystats then
        ...
        local budgetMs = (ofs3.performance and ofs3.performance.budget_ms) or 50  -- 20 Hz -> 50ms
        local tickMs   = (ofs3.performance and ofs3.performance.tick_ms)   -- optional, if you published it
        ...
        local schedRows = {
            ...
            { "LOAD (100ms window)", fmtPct(ofs3.performance.cpuload_window100 or 0), "%" },
...
function dashboard.wakeup(widget)
    ...
    if firstWakeupCustomTheme and
        ofs3.session.mcu_id and
        ofs3.session.modelPreferences and
        ofs3.session.modelPreferences.dashboard then
...
function dashboard.savePreference(key, value)
    if not ofs3.session.modelPreferences or not ofs3.session.modelPreferencesFile or not dashboard.currentWidgetPath then
        return false
    end
    if not ofs3.app.guiIsRunning then
