...
if not ofs3.ini.ini_tables_equal(master_ini, updated_ini) then ofs3.ini.save_ini_file(userpref_file, updated_ini) end
...
local function register_main_tool() system.registerSystemTool({event = ofs3.app.event, name = ofs3.config.toolName, icon = ofs3.config.icon, create = ofs3.app.create, wakeup = ofs3.app.wakeup, paint = ofs3.app.paint, close = ofs3.app.close}) end

local function register_bg_task() system.registerTask({name = ofs3.config.bgTaskName, key = ofs3.config.bgTaskKey, wakeup = ofs3.tasks.wakeup, event = ofs3.tasks.event, init = ofs3.tasks.init}) end
