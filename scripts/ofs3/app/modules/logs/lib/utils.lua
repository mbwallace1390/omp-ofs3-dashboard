...
function utils.resolveModelName(foldername)
    ...
    local iniName = "LOGS:ofs3/telemetry/" .. foldername .. "/logs.ini"
...
function utils.getLogPath()
    ...
    --if ofs3.session.activeLogDir then
    --    return string.format("LOGS:/ofs3/telemetry/%s/", ofs3.session.activeLogDir)
    --end
    return "LOGS:/ofs3/telemetry/"
end
...
function utils.getLogDir(dirname)
    ...
    if not dirname then
        local defaultDir = "LOGS:/ofs3/telemetry/" 
        os.mkdir(defaultDir)
        return defaultDir
    end
