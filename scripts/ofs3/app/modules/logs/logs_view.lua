...
graphPos['width'] = math.floor(LCD_W * ofs3.app.radio.logGraphWidthPercentage)
...
local logColumns = ofs3.tasks.logging.getLogTable()
...
local function readNextChunk()
    ...
    if chunk then
        ...
        ofs3.utils.log("Read " .. #chunk .. " bytes from log file", "debug")
    else
        logFileHandle:close()
        logFileHandle = nil
...
local function drawKey(name, keyunit, keyminmax, keyfloor, color, minimum, maximum, laneY, laneHeight)
    ...
    lcd.font(ofs3.app.radio.logKeyFont)
    ...
    lcd.font(ofs3.app.radio.logKeyFontSmall)
    ...
    if ofs3.app.radio.logShowAvg == true then
...
local function drawCurrentIndex(points, position, totalPoints, keyindex, keyunit, keyfloor, name, color, laneY, laneHeight, laneNumber, totalLanes)
    ...
    local sliderPadding = ofs3.app.radio.logSliderPaddingLeft
...
local function openPage(pidx, title, script, logfile, displaymode, dirname)
    ...
    ofs3.app.formFields[1] = form.addSliderField(nil, posField, 0, 100, function() return sliderPosition end, function(newValue) sliderPosition = newValue end)
    ...
    ofs3.app.formFields[2] = form.addButton(line, posField, {
        ...
        press = function()
            if zoomLevel > 1 then
                ...
                lcd.invalidate()
                ofs3.app.formFields[2]:enable(true)
                ofs3.app.formFields[3]:enable(true)
            end
            if zoomLevel == 1 then
                ofs3.app.formFields[2]:enable(false)
                ofs3.app.formFields[3]:focus()
            end
        end
    })
    ...
    ofs3.app.formFields[3] = form.addButton(line, posField, {
        ...
        press = function()
            ...
            if zoomLevel == zoomCount then
                ofs3.app.formFields[3]:enable(false)
                ofs3.app.formFields[2]:focus()
            end
        end
    })

    ofs3.app.formFields[1]:step(1)
...
local function event(event, category, value, x, y)
    if value == 35 then
        ofs3.app.ui.openPage(ofs3.app.lastIdx, ofs3.app.lastTitle, "logs/logs_logs.lua")
        return true
    end
    return false
end
...
local function wakeup()
    ...
    if logDataRawReadComplete and not processedLogData then
        ...
        if currentDataIndex >= #logColumns then
            ...
            local btnMinus = ofs3.app.formFields[2]
            local btnPlus = ofs3.app.formFields[3]
            if zoomCount <= 1 then
