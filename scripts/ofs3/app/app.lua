...
function app.updateTelemetryState()
    ...
    if system:getVersion().simulation ~= true then
        if not ofs3.session.telemetrySensor then
...
A table containing a sequence of functions, each representing a logical UI update or task for the ofs3 Ethos Suite application. These tasks are executed in order to manage UI state, handle dialogs, process telemetry, and respond to user or system triggers.
...
app._uiTasks = {
  ...
  function()
    ...
    if app.uiState == app.uiStatus.mainMenu then
      invalidatePages()
    elseif app.triggers.isReady and ofs3.tasks.msp.mspQueue:isProcessed()
...
function app.event(widget, category, value, x, y)
    ...
    if app.uiState == app.uiStatus.pages then
        ...
        if value == KEY_ENTER_LONG then
            if ofs3.app.Page.navButtons and ofs3.app.Page.navButtons.save == false then
                return true
            end
...
function app.close()
    ...
    ofs3.utils.reportMemoryUsage("closing application: start")

    -- save user preferences
    local userpref_file = "SCRIPTS:/" .. ofs3.config.preferences .. "/preferences.ini"
    ofs3.ini.save_ini_file(userpref_file, ofs3.preferences)
    ...
    ofs3.app.triggers.profileswitchLast = nil
    ...
    --for i,v in pairs(ofs3.app) do
    --        log("   ->" .. tostring(i) .. " = " .. tostring(v), "info")
    --end

    ofs3.utils.reportMemoryUsage("closing application: end")    
