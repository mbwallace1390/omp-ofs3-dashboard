...
function utils.getRSSI()
    ...
    if ofs3.simevent.rflink == 1 then return 0 end

    if ofs3.app.offlineMode == true then return 100 end

    if ofs3.session.telemetryState then
        return 100
    else
        return 0
    end
...
function utils.getFieldValue(f)
    ...
    if f.decimals then v = ofs3.utils.round(v * ofs3.app.utils.decimalInc(f.decimals), 2) end
...
function utils.saveFieldValue(f, value)
    if value then
        ...
        if f.decimals then
            f.value = value / ofs3.app.utils.decimalInc(f.decimals)
        else
            f.value = value
        end
        if f.postEdit then f.postEdit(ofs3.app.Page) end
    end
...
function utils.scaleValue(value, f)
    ...
    local v = value * ofs3.app.utils.decimalInc(f.decimals)
    if f.scale then v = v / f.scale end
    return ofs3.utils.round(v)
end
...
function utils.getInlinePositions(f, lPage)
    ...
    local inline_size = utils.getInlineSize(f.label, lPage) * ofs3.app.radio.inlinesize_mult
    ...
    local eH = ofs3.app.radio.navbuttonHeight
