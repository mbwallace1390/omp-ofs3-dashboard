...
function ui.progressDisplaySave(message)
    ...
    if not message then message = "@i18n(app.msg_saving_to_fbl)@" end
...
function ui.progressDisplaySaveValue(value, message)
    if value >= 100 then
        if ofs3.app.dialogs.save then ofs3.app.dialogs.save:value(value) end
...
function ui.progressDisplayCloseAllowed(status)
    local progress = ofs3.app.dialogs.progress
    if progress then progress:closeAllowed(status) end
end

function ui.progressDisplayMessage(message)
    local progress = ofs3.app.dialogs.progress
    if progress then progress:message(message) end
end
...
function ui.enableAllFields() for _, field in ipairs(ofs3.app.formFields) do if type(field) == "userdata" then field:enable(true) end end end

function ui.disableAllNavigationFields() for i, v in pairs(ofs3.app.formNavigationFields) do if x ~= v then v:enable(false) end end end

function ui.enableAllNavigationFields() for i, v in pairs(ofs3.app.formNavigationFields) do if x ~= v then v:enable(true) end end end
...
function ui.progressDisplayIsActive() return ofs3.app.dialogs.progressDisplay or ofs3.app.dialogs.saveDisplay or ofs3.app.dialogs.progressDisplayEsc or ofs3.app.dialogs.nolinkDisplay or ofs3.app.dialogs.badversionDisplay end
...
function ui.openMainMenu()
    ...
    for i in pairs(ofs3.app.gfx_buttons) do if i ~= "mainmenu" then ofs3.app.gfx_buttons[i] = nil end end

    if not ofs3.utils.ethosVersionAtLeast(config.ethosVersion) then return end
    ...
    for idx, section in ipairs(MainMenu.sections) do
        local hideSection = (section.ethosversion and ofs3.session.ethosRunningVersion < section.ethosversion) or (section.mspversion and (ofs3.session.apiVersion or 1) < section.mspversion) or (section.developer and not ofs3.preferences.developer.devtools)
        ...
        if not hideSection then
            ...
            for pidx, page in ipairs(MainMenu.pages) do
                if page.section == idx then
                    local hideEntry = (page.ethosversion and not ofs3.utils.ethosVersionAtLeast(page.ethosversion)) or (page.mspversion and (ofs3.session.apiVersion or 1) < page.mspversion) or (page.developer and not ofs3.preferences.developer.devtools)
                    ...
                    if not hideEntry then
                        ...
                        if ofs3.preferences.menulastselected["mainmenu"] == pidx then ofs3.app.formFields[pidx]:focus() end
...
function ui.fieldChoice(i)
    ...
    formFields[i] = form.addChoiceField(formLines[ofs3.session.formLineCnt], posField, tbldata, function()
...
function ui.fieldNumber(i)
    ...
    formFields[i] = form.addNumberField(formLines[ofs3.session.formLineCnt], posField, minValue, maxValue, function()
...
function ui.fieldText(i)
    ...
    formFields[i] = form.addTextField(formLines[ofs3.session.formLineCnt], posField, function()
        if not fields or not fields[i] then
            ui.disableAllFields()
            ui.disableAllNavigationFields()
            ui.enableNavigationField('menu')
...
function ui.openPageRefresh(idx, title, script, extra1, extra2, extra3, extra5, extra6) ofs3.app.triggers.isReady = false end
...
function ui.openPage(idx, title, script, extra1, extra2, extra3, extra5, extra6)
    ...
    if ofs3.app.Page.headerLine then
        ...
        form.addStaticText(headerLine, {x = 0, y = ofs3.app.radio.linePaddingTop, w = ofs3.session.lcdWidth, h = ofs3.app.radio.navbuttonHeight}, ofs3.app.Page.headerLine)
    end
    ...
    if ofs3.app.Page.fields then
        for i, field in ipairs(ofs3.app.Page.fields) do
            ...
            local valid = (field.apiversion == nil or ofs3.utils.round(field.apiversion, 2) <= version) and (field.apiversionlt == nil or ofs3.utils.round(field.apiversionlt, 2) > version) and (field.apiversiongt == nil or ofs3.utils.round(field.apiversiongt, 2) < version) and
                              (field.apiversionlte == nil or ofs3.utils.round(field.apiversionlte, 2) >= version) and (field.apiversiongte == nil or ofs3.utils.round(field.apiversiongte, 2) <= version) and (field.enablefunction == nil or field.enablefunction())
...
function ui.openPageDashboard(idx, title, script, source, folder)
    ...
    ofs3.app.formNavigationFields['menu'] = form.addButton(line, {x = x, y = ofs3.app.radio.linePaddingTop, w = buttonW, h = ofs3.app.radio.navbuttonHeight}, {
        ...
        press = function()
            ...
            ofs3.app.ui.openPage(pageIdx, "@i18n(app.modules.settings.dashboard)@", "settings/tools/dashboard_settings.lua")
        end
    })
            ...
            local buttons = {
                {
                    ...
                    action = function()
                        ...
                        if ofs3.app.Page.write then ofs3.app.Page.write() end
...
function ui.navigationButtons(x, y, w, h)
    ...
    if navButtons.tool ~= nil and navButtons.tool == true then
        ofs3.app.formNavigationFields['tool'] = form.addButton(line, {x = toolOffset, y = y, w = wS, h = h}, {text = "@i18n(app.navigation_tools)@", icon = nil, options = FONT_S, paint = function() end, press = function() ofs3.app.Page.onToolMenu() end})
    end
    ...
    if navButtons.help ~= nil and navButtons.help == true then
        ...
        if help then
            ...
            ofs3.app.formNavigationFields['help'] = form.addButton(line, {x = helpOffset, y = y, w = wS, h = h}, {text = "@i18n(app.navigation_help)@", icon = nil, options = FONT_S, paint = function() end, press = function() end})
...
function ui.openPageHelp(txtData, section)
    ...
    form.openDialog({width = ofs3.session.lcdWidth, title = "Help - " .. ofs3.app.lastTitle, message = message, buttons = {{label = "@i18n(app.btn_close)@", action = function() return true end}}, options = TEXT_LEFT})
end
