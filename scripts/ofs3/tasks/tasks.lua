...
function tasks.telemetryCheckScheduler()
    ...
    if system.getVersion().simulation and ofs3.simevent.telemetry_state == false then
...
function tasks.wakeup()
  ...
  last_wakeup_start = now
    ...
    if (now2 - last_mem_t) >= MEM_PERIOD then
        ...
        ofs3.session.freeram   = mem_avg_kb          -- KB (ema)
        ...
        ofs3.session.freeram   = mem_avg_kb          -- may be nil if never known
...
-- After updating all tasks, invokes the `ofs3.utils.session()` function.
function tasks.setTelemetryTypeChanged()
    for _, task in ipairs(tasksList) do
        if tasks[task.name].setTelemetryTypeChanged then
            --ofs3.utils.log("Notifying task [" .. task.name .. "] of telemetry type change", "info")
