...
-- 
-- This function retrieves the user's switch preferences from `ofs3.preferences.switches`.
...
-- @usage
--   initializeSwitches()
-- @see ofs3.preferences.switches
-- @see system.getSource
-- @see ofs3.tasks.telemetry.listSensorAudioUnits
local function initializeSwitches()
    local prefs = ofs3.preferences.switches
    ...
    switchTable.units = ofs3.tasks.telemetry.listSensorAudioUnits()
end
...
--   - `ofs3.tasks.telemetry.getSensorSource(key)`: Retrieves the sensor source for a switch.
...
function switches.wakeup()
    ...
    for key, sensor in pairs(switchTable.switches) do
        ...
        if playNow then
            local sensorSrc = ofs3.tasks.telemetry.getSensorSource(key)
            if sensorSrc then
